...
admin: '',
    ...
    methods: {
        ...
        loadData:function(){
            ...
            this.$http
                .GetOldStudentById(this.$route.params.id)
                .then(response => {
...
this.formData.graduation_year=response.data.data.graduation_year;
this.formData.certificate_type=response.data.data.certificate_type;
this.formData.graduation_degree=response.data.data.graduation_degree;
...
this.validateAdmin();
 if (this.formValidate.admin != '') return 0;
        ...
        /*this.$loading.Start();*/ this.$store.commit("loadingStart");
            this.$http
            .EditOldStudent(this.$route.params.id,this.formData)
            .then(response => {
                /*this.$loading.Stop();*/ this.$store.commit("loadingStop");
                if (response.status == 200) {
                    this.$alert.Success(response.data.message);
